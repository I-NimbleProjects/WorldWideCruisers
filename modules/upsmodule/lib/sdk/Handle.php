...
class Handle
{
    ...
    protected $time_request = null;
    protected $time_response = null;
    ...
    {
        $this->time_response = date("Y-m-d H:i:s");
        $this->response = json_decode($response);
        return $this->response;
    }

    protected function writeLogsApiDB()
    {
        if (!LOG_WRITER || $this->path == 'Merchant/WriteLogger') {
            return;
        }

        $sql = "INSERT INTO `" . _DB_PREFIX_ . "ups_logs_api`" .
            "(`method`, `full_uri`, `request`, `response`, `time_request`, `time_response`) " .
            "VALUES ('" . $this->path . "', '" . $this->endpoint ."', '" .
            pSQL(json_encode($this->request)) . "', '" . pSQL(json_encode($this->response)) . "', '" .
            $this->time_request . "', '" . $this->time_response . "');";

        $this->sdk->dbInstance->execute($sql);
    }

    protected function saveLogAPI()
    {
        $params = [];
        $params['Platform'] = '10';
        ...
        $params['LogApiUrl'] = $this->endpoint;
        $params['LogApiRequest'] = json_encode($this->request);
        $params['LogApiResponse'] = json_encode($this->response);
