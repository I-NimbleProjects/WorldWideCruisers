...
class AdminSpringxbsApiController extends ModuleAdminController
{
    ...
    {
        $stamp = (int)SpringXbsHelper::get(Springxbs::API_SPRING_TRACK_LABELS_STAMP)
        ...
        echo "Runned on" . " :" . _PS_VERSION_ . "|1.4.4\n";
        ...
        foreach ($rows as $row) {
            echo SpringXbsHelper::l("processing") . " " . (++$cnt) . ":$row[tracking_number]\n";
            flush();
    ...
    private function printLabel($id_order)
    {
        ...
        if (!$shipment) {
            exit(SpringXbsHelper::l('The shipment is not ordered yet'));
        }
    ...
    public function ajaxProcessPrintZprnLabel()
    {
        ...
        if (!$shipment['label']) {
            exit(SpringXbsHelper::l('Shipment not ordered yet'));
        }
    ...
    public function orderShipment($id_order, $output = true)
    {
        ...
        if (!Validate::isLoadedObject($order)) {
            return array(
                'error' => SpringXbsHelper::l('Carrier not allowed'),
        ...
        if (!$service_name) {
            return array(
                'error' => SpringXbsHelper::l('Cannot create label: Service type error'),
                'test_mode' => SpringXbsHelper::get(Springxbs::MODULE_TEST_MODE_NAME),
            );
        }
        ...
        if ($shipment && !$output) {
            return array(
                'error' => SpringXbsHelper::l('Nothing done'),
                'shipment' => $shipment,
                'test_mode' => SpringXbsHelper::get(Springxbs::MODULE_TEST_MODE_NAME),
            );
        } elseif ($shipment) {
        ...
        $this->calculateProductWeight($orderProducts, (float)$dimensions['weight']);
        ...
        foreach ($orderProducts as $product) {
            ...
            if (!empty($products_data) && !empty($products_data[$product['id_product']])) {
...
//                "PurchaseUrl" => "http://url.com/book1",
                    ...
                    "Value" => ($dimensions['ddp'] === 'DDP')
                        ? static::round2f($product['total_price_tax_excl'])
                        : static::round2f($product['total_price_tax_incl']),
                ) + $coo_arr;

            $proArr['Weight'] = $product['ddpWeight'];
        ...
        $shipment = array(
            ...
            "Weight" => static::round1f(max($dimensions['weight'], self::DEFAULT_MIN_WEIGHT)),
        ...
        if ($dimensions['ddp'] === 'DDP') {
            $shipment['CustomsDuty'] = $dimensions['ddp'];
            $shipment['ShippingValue'] = static::round2f($order->total_shipping_tax_excl);
            $shipment['Value'] += $shipment['ShippingValue'];
        }
        ...
        if (empty($api_result['Shipment']['TrackingNumber']) ||
            ...
            return array(
                'error' => SpringXbsHelper::l('Fatal error, try later'),
                'shipment' => $shipment,
                'api_result' => $api_result,
                'test_mode' => SpringXbsHelper::get(Springxbs::MODULE_TEST_MODE_NAME),
            );
        ...
        return array(
            'success' => SpringXbsHelper::l('Label generated successfully'),
            'shipment' => $shipment,
            'api_result' => $api_result,
            'test_mode' => SpringXbsHelper::get(Springxbs::MODULE_TEST_MODE_NAME),
        );
    ...
    protected function updateOrderState(Order &$order, $status_code)
    {
        ...
        if (isset($_map[$status_code]) && $_map[$status_code] > 0 && // something selected
            $_map[$status_code] != $order->current_state && // other status
            SpringXbsHelper::get(Springxbs::MODULE_ENABLE_EXTERNAL_TRACKING)) { // status editing on
            if ($this->debug) {
                echo SpringXbsHelper::l("Current status") . ": " . $order->current_state . "\n";
                echo SpringXbsHelper::l("New status") . ": " . $_map[$status_code] . " ($status_code) \n";
                echo SpringXbsHelper::l("Employee ID") . ": " . (Context::getContext()->employee->id) . "\n";
            }
            ...
            if (!Context::getContext()->employee->id) {
                ...
                echo SpringXbsHelper::l("Recovered Employee") . ": " . (Context::getContext()->employee->id) . "\n";
            }
    ...
    protected function getLabelByApi($id_order, $service_name, $label_format)
    {
        ...
        if (!$shipment) {
            exit(SpringXbsHelper::l('The shipment is not exists'));
        }
    ...
    public function trackShipment($id_order, $state_out = false)
    {
        ...
        foreach ($tracking_items as $i => $shipment_data) {
            if ($state_out) {
                echo SpringXbsHelper::l("Start") . " $i ($id_order|-|$shipment[status_code])"
                    . reset($shipment_data) . "\n";
            }
            ...
            if (!isset($api_result['Error']) && isset($api_result['Shipment']['Events'])) {
                ...
                if ($state_out && !$i && ($api_result['Shipment']['TrackingNumber'] != $shipment['tracking_number'] ||
                    ...
                    echo SpringXbsHelper::l("New tn") . ':' . $api_result['Shipment']['TrackingNumber'] . "\n";
                    $this->reorder_labels[$id_order] = $api_result['Shipment']['TrackingNumber'];
                }

                if ($state_out) {
                    echo SpringXbsHelper::l("Tracking number") . ':' . $api_result['Shipment']['TrackingNumber']
                        . " - " . SpringXbsHelper::l("data received") . "\n";
                }
                ...
                if ($last_event) {
                    ...
                    if ($state_out) {
                        echo SpringXbsHelper::l("Before order state") . "\n";
                    }

                    try {
                        $this->updateOrderState($order, $status_code);
                    } catch (Exception $e) {
                        echo SpringXbsHelper::l('Exeption-updateOrderState') . ':' . $e->getMessage() . "\n";
                    }

                    if ($state_out) {
                        echo SpringXbsHelper::l("After order state") . "\n";
                    }
                }
    ...
    public static function getShipmentDimensions($id_order)
    {
        ...
        return SpringTools::normalizeDimensions(array(
            ...
            'ddp' => Tools::strtoupper($shipment['request_shipment']['CustomsDuty']),
        ));
    }
    ...
    protected function getApiData($data, $test_mode)
    {
        ...
        if (!empty($response['error']) ||
            ...
            $error_msg = SpringXbsHelper::l("Fatal error, command is not complete");
    ...
    public function ajaxProcessChangeOrderCarrier()
    {
        ...
        $order->update();
        Tools::redirectAdmin(
            Context::getContext()->link->getAdminLink('AdminOrders') . "&id_order=$id_order"
        );
    }
    ...
    /**
     * Prepare data for DDP case
     *
     * @param array $orderProducts
     * @param float $userWeight
     */
    protected function calculateProductWeight(&$orderProducts, $userWeight)
    {
        $notAllNullWeight = false;
        $hasNullWeight = false;
        $noUserWeight = false;
        $totalWeight = 0;
        $totalQuantity = 0;
        ...
        foreach ($orderProducts as &$orderProduct) {
            $totalWeight += $orderProduct['weight'] * $orderProduct['product_quantity'];
            $totalQuantity += $orderProduct['product_quantity'];

            if ($orderProduct['weight'] <= 0.0009) {
                $orderProduct['isNull'] = true;
                $hasNullWeight = true;
            } else {
                $orderProduct['isNull'] = false;
                $notAllNullWeight = true;
            }
        }
        unset($orderProduct);

        if ($userWeight <= 0.0009) {
            $noUserWeight = true;

            if ($totalWeight <= 0.0009) {
                $userWeight = 1.0;
            }
        }

        if (!$hasNullWeight) {
            if ($totalWeight === $userWeight || $noUserWeight) {
                foreach ($orderProducts as &$orderProduct) {
                    $orderProduct['ddpWeight'] = $orderProduct['weight'] * $orderProduct['product_quantity'];
                }
                unset($orderProduct);
            } else {
                foreach ($orderProducts as &$orderProduct) {
                    $orderProduct['ddpWeight'] = ($orderProduct['weight'] * ($userWeight / $totalWeight))
                        * $orderProduct['product_quantity'];
                }
                unset($orderProduct);
            }
        } elseif ($notAllNullWeight) {
            $perItemWeight = ($userWeight ?: $totalWeight) / $totalQuantity;

            foreach ($orderProducts as &$orderProduct) {
                $orderProduct['ddpWeight'] = $perItemWeight * $orderProduct['product_quantity'];
            }
            unset($orderProduct);
        } else {
            foreach ($orderProducts as &$orderProduct) {
                $orderProduct['ddpWeight'] = ($userWeight / $totalQuantity) * $orderProduct['product_quantity'];
            }
            unset($orderProduct);
        }
    }
     ...
     */
    public function ajaxProcessBulkRequestLabels()
    {
        $this->ajax = true;
        $id_orders = explode(',', Tools::getValue('id_orders'));
        $results = [];

        foreach ($id_orders as $id_order) {
            $result = $this->orderShipment($id_order, false);
            $results[] = [$id_order => $result];
        }
