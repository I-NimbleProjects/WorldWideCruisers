...
class AdminUpsOpenOrdersController extends CommonController
{
    ...
    public function __construct()
    {
         ...
         * #22-08-2018
         */
        $this->_where = "AND op.status = " . Constants::STATUS_OPEN_ORDER . " AND a.current_state NOT IN ( "
                            . Constants::STATUS_SHIPPED . ','
                            . Constants::STATUS_DELIVERED . ','
                            . Constants::STATUS_ORDER_CANCELED . ','
                            . Constants::STATUS_REFUNDED .
                        ")";
    ...
    public function setMedia($isNewTheme = false)
    {
         ...
         * #22-08-2018
         */
        $sql->where("op.status = '1' AND o.current_state NOT IN ("
            . Constants::STATUS_SHIPPED . ','
            . Constants::STATUS_DELIVERED . ','
            . Constants::STATUS_ORDER_CANCELED . ','
            . Constants::STATUS_REFUNDED .
        ...
        if (!empty($orderIds)) {
            ...
            if (!empty($proccessingOrders)) {
                if ($this->hasSameShippingMethods($proccessingOrders) || $isBatch) {
                    ...
                    $listPackage = [];
                    if ($firstOrder['package']) {
                        $listPackage = unserialize($firstOrder['package']);
                    }
                    $returnOrders['listPackage']     = $listPackage;
    ...
    public function Processorder($orderIds)
    {
        $returnOrderData = array();

        if (!empty($orderIds)) {
            $proccessingOrders = $this->getOrders($orderIds);
            $returnOrderData['errors'] = false;
            $returnOrderData['tmpOrderId'][] = $orderIds;
  
            if (!empty($proccessingOrders)) {
                $returnOrderData['orders'] = $proccessingOrders;
                $shippingService = "";
                $shippingMethod = "";
                $accountDefault = "";
                $codAccessorial = "";
                $serviceTypeName = "";
                if ($this->hasSameShippingMethods($proccessingOrders)) {
                    $firstOrder = $proccessingOrders[0];
                    ...
                    $returnOrderData['firstOrder']      = $firstOrder;
                    $returnOrderData['accountNumber']  = $accountDefault;
                    $returnOrderData['packages']     = unserialize($firstOrder['package']);
                    $returnOrderData['accessorialKeys'] = array();
                    $returnOrderData['accessorialsService'] = array();
                    $returnOrderData['isApCod'] = isset($isApCod) ? $isApCod : '';
                } else {
                    $returnOrderData['errors'] = "Multiple orders with different Shipping methods (send to Access Point and to Address) can not be merged";
                }
            }
        } else {
            $returnOrderData['errors'] = $this->sdk->t('err-msg', 'selectRequest');
        }
    ...
    public function initModal()
    {
        ...
        if ($this->module->usa()) {
            ...
            $txtErrorMgs = $sdk->t('err-msg', 'notValidUS');
        } else {
            $txtE_ShippingMethod = $sdk->t('ups', 'txtE_ShippingMethod');
            $txtErrorMgs = $sdk->t('err-msg', 'notValid');
        }
        ...
        $texts = array(
            ...
            // 'txtDownloadUPSLabel'            => $sdk->t('openorder', 'txtDownloadUPSLabel'),
            ...
            if (!empty($orders)) {
                foreach ($orders as $order) {
                    ...
                    if (empty($order['package'])) {
                        ...
                        $packages = $this->formatOrderPackageNew(unserialize($order['package']));
                    }
                    ...
                    if ($result['Code'] == 1) {
                        ...
                        CommonFunction::sqlUpdateOrderStatus(Constants::STATUS_SHIPPED, array($order['id_order']));
                        CommonFunction::sqlUpdateTrackingNumber($result['data'][0]['ShipmentId'], array($order['id_order']));
            ...
            if (!empty($orders)) {
                ...
                $result = $this->createShipment(
                    array(
                        ...
                        'accountNumber'       => $accountNumber,
                        'packages'            => Tools::getValue('packageDetail'),
    ...
    private function esimateTimeFee(
        ...
        $rateParams['PackageWeightCode']        = $packages[0]['unit_weight'];
        $rateParams['PackageWeightDescription'] = $packages[0]['unit_weight'];
        $rateParams['PackageWeightWeight']      = $packages[0]['weight'];
    ...
    public function getOrders($orderIds)
    {
        ...
        $sql->select('op.ap_city as ap_city');
        $sql->select('op.accessorials_service as accessorials_service');
        $sql->select('op.package as package');
    ...
    private function getAllOpenOrderIds()
    {
        $id_lang     = (int) $this->context->language->id;
        $statuses = array(
            Constants::STATUS_ARCHIVED_ORDERS,
            Constants::STATUS_SHIPPED,
            Constants::STATUS_DELIVERED,
            Constants::STATUS_ORDER_CANCELED,
            Constants::STATUS_REFUNDED
        ...
        if (version_compare(_PS_VERSION_, '1.7', '<')) {
            ...
            $whereClause = 'WHERE 1 AND op.status = 1
                            AND a.current_state NOT IN (' . implode(",", $statuses) . ')
                            GROUP BY a.id_order';
        } else {
