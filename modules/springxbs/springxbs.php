...
class Springxbs extends Module
{
    ...
    public function hookDisplayAdminOrder($params)
    {
        ...
        if ($shipment && $shipment['error_level']) {
            ...
            $shipment_data = json_decode($shipment['data'])
                ->response_shipment
                ->DdpInfo
                ->landedCost;
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submit' . $this->name)) {
            if (Tools::getValue(self::IS_API_SETTINGS_TAB)) {
                ...
                SpringXbsHelper::setNotification($this->l('API settings updated') . ' ');
                ...
                SpringXbsHelper::setNotification($this->l('Labels settings updated'));
            } elseif (Tools::getValue(self::IS_NOTIFICATIONS_SETTINGS_TAB)) {
                $_map = array();

                foreach (array_keys(self::getTrackingEvents()) as $code) {
                    $_map[$code] = Tools::getValue(self::SPRINGXBS_TO_SHOP_STATUS_MAP . '_' . $code);
                }
                ...
                SpringXbsHelper::setNotification($this->l('Notifications settings updated'));
                ...
                SpringXbsHelper::setNotification($this->l('Advanced settings updated'));
                ...
                if ($hs_code) {
                    if (SpringTools::validateHSCode($hs_code)) {
                        ...
                        $l = 'Invalid Default HS code. Should contain at least 6 digits. i.e.: 999900 or 9999.00';
                        SpringXbsHelper::setNotification(
                            $this->l($l),
                            SpringXbsHelper::MSG_TYPE_ERROR
                        );
                ...
                SpringXbsHelper::setNotification($this->l('Customs information updated'));
            }
    ...
    public function importProductsCatalog()
    {
        if (empty($_FILES['csv_file']['tmp_name'])) {
            SpringXbsHelper::setNotification($this->l("No file uploaded"), SpringXbsHelper::MSG_TYPE_OK);
            return;
        }
    ...
    public function getExtraTranslations()
    {
        return array(
            ...
            $this->l('Label generated successfully'),
        );
    }
    ...
    public function hookDisplayAdminListAfter($params)
    {
        ...
        if (Tools::getValue('controller') === 'AdminOrders' && !Tools::getValue('id_order')) {
            ...
            $admLink = $this->context->link->getAdminLink('AdminSpringxbsApi');

            if (!stripos($admLink, basename(_PS_ADMIN_DIR_))) {
                $admLink = '/' . basename(_PS_ADMIN_DIR_) . '/' . $admLink;
            }
