...
class Validator
{
    ...
    public function validUserID($UserID, $key)
    {
        if ($this->isBlank($key, $UserID)) {
            return $key;
        }
    }

    public function validPwd($Pwd, $key)
    {
        if ($this->isBlank($key, $Pwd)) {
            return $key;
        }
    }

    public function validAcssKey($AcssKey, $key)
    {
        if ($this->isBlank($key, $AcssKey)) {
            return $key;
        }
    }

    public function validUPSUsername($UPSUsername, $key)
    {
        if ($this->isBlank($key, $UPSUsername)) {
            return $key;
        }
    }

    public function validUPSPassword($UPSPassword, $key)
    {
        if ($this->isBlank($key, $UPSPassword)) {
            return $key;
        }
    }
}
