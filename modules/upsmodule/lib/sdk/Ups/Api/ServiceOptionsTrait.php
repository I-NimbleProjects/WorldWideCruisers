...
trait ServiceOptionsTrait
{
    ...
    public function packageHandle($packages, $accessorials, $country = null)
    {
        ...
        {
            $unitLength = isset($package['unit_dimension']) ? trim(strtoupper($package['unit_dimension'])) : "";
            if ('INCH' == $unitLength) {
                $unitLength = 'IN';
            }
            ...
            $tmp["Dimensions"] = [
                "UnitOfMeasurement" => [
                    "Code" => $unitLength,
                    "Description" => strtoupper($package['unit_dimension']),
                ],
            ...
            $tmp["PackageWeight"] = [
                "UnitOfMeasurement" => [
                    "Code" => strtoupper($package['unit_weight']),
                    "Description" => strtoupper($package['unit_weight']),
                ],
    ...
    public function shipmentServiceOptionsHandle(
        ...
        $locale = $this->mappingDialectLanguage($countryCodeEshoper);
    ...
    {
        $countryCodeUpper = strtoupper($country);
        //language
        $selectedLanguage = 'ENG';
        $arrayKey = array_keys($this->listCountryLanguages);
        if (in_array($countryCodeUpper, $arrayKey)) {
            $selectedLanguage = $this->listCountryLanguages[$countryCodeUpper];
        }
        // dialect
        $selectedDialect = 'GB';
        if (in_array($countryCodeUpper, $this->listCountryDialects)) {
            $selectedDialect = '97';
        }
        if ('RO' == $countryCodeUpper) {
            $selectedDialect = 'RO';
        }
        return [
