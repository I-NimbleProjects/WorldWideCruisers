...
$(document).ready(function() {
    ...
    if (typeof $("label[for='delivery_option_" + idCarrier + "'] .carrier-price") == 'object' && $("label[for='delivery_option_" + idCarrier + "'] .carrier-price").length > 0) {
        ...
        var shipping_time_format = ' ';
        var value_time = $('input#delivery_option_' + idCarrier).next().find("span").html();
        value_time = value_time.trim();
        if (value_time) {
            shipping_time_format = cheapestTime;
        }

        var value_format = $('input#delivery_option_' + idCarrier).next().find("span").next().html();
        if (value_format.lastIndexOf("tax incl.") > 0) {
            cheapestPrice += ' tax incl.';
        }
        $('input#delivery_option_' + idCarrier).next().find("span").html(shipping_time_format);
        $('input#delivery_option_' + idCarrier).next().find("span").next().html(cheapestPrice);
    }
    ...
    $('input[type=radio][name=optradio]').change(function() {
        ...
        if (typeof $("label[for='delivery_option_" + idCarrier + "'] .carrier-price") == 'object' && $("label[for='delivery_option_" + idCarrier + "'] .carrier-price").length > 0) {
            ...
            value_time = value_time.trim();
            if (value_time) {
                shipping_time_format = shipping_time;
            }

            var value_format = $('input#delivery_option_' + idCarrier).next().find("span").next().html();
            if (value_format.lastIndexOf("tax incl.") > 0) {
                shipping_value_format += ' tax incl.';
            }
            $('input#delivery_option_' + idCarrier).next().find("span").html(shipping_time_format);
            $('input#delivery_option_' + idCarrier).next().find("span").next().html(shipping_value_format);
        }
    });
