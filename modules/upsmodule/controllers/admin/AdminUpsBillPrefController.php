...
class AdminUpsBillPrefController extends CommonController
{
    ...
    private function drawHtml()
    {
        ...
        $ttlNegotiatedRates    = $sdk->t('billing', 'txtNegotiatedRates');
        $contentNegotiatedRatesSub   = $sdk->t('billing', 'txtNegotiatedRatesSub');
        $contentUserId   = $sdk->t('billing', 'txtUserId').'['.Configuration::get('MY_UPS_ID').'].';
       
        $contentUserIdSub   = $sdk->t('billing', 'txtUserIdSub').'['.Configuration::get('MY_UPS_ID').'].'. $sdk->t('billing', 'txtUserIdSub2') ;

        $txtSearch      = $sdk->t('ups', $keys['txtSearchAP']);

        $html = "<h5 class='content-bold'>$titleInfo</h5>
        <p class='pad-paragraph'>$desc</p>
        <h5 class='content-bold'>$ttlNegotiatedRates</h5>
        <p class='pad-paragraph'>$contentNegotiatedRatesSub</p>
        <p class='pad-paragraph'>$contentUserId</p>
        <p class='pad-paragraph'>$contentUserIdSub</p>
