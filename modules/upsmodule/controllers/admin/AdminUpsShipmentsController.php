    ...
    public function ajaxProcessPrintLabel()
    {
        ...
        if (!empty($trackingNumbers)) {
            ...
            foreach ($arrShipmentId as $trackingNum) {
                ...
                $fileExt = '.' . Tools::strtolower($labelFormat);
                $decoded = '';
                ...
                if(count($arrShipmentId) == 1) {
                    $label_format = Tools::getValue('labelFormat');
                    ...
                    if ($redirect) {
                        ...
                        $return['error'] = '';
                        $return['link']  = $this->context->link->getAdminLink('AdminUpsShowBarcodeImage'). '&labelFormat='.$label_format;
                    } else {
            ...
            if ($result && $redirect) {
                Commonfunction::removeFiles($filePaths);
                $return['error'] = '';
                $return['link']  = $this->context->link->getAdminLink('AdminUpsShowBarcodeImage') .'&labelFormat=.zip';
            } else {
                $return['error'] = $this->sdk->t('shipment', 'msgLabel');
                $return['link']  = '';
            }
