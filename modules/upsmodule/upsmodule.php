...
class UpsModule extends Module
{
    ...
    public function __construct()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.7.8') == -1) {
    ...
    private function initConfig()
    {
        ...
        $this->sdk->setMerchantKey(Configuration::get('MERCHANT_KEY'));
        $this->sdk->setCountryCode(Configuration::get('UPS_COUNTRY_SELECTED'));
        $this->sdk->setRegisteredToken(Configuration::get('REGISTERED_TOKEN'));
    }
    ...
    {
        $ap_shipto = Configuration::get('UPS_SP_SERV_AP_SHIP_TO_ADDR');

        if(!empty($ap_shipto) && $ap_shipto == 1 ){
            $context = Context::getContext();
            ...
            if ($this->checkServiceType($shippingService) == 'AP' && !empty($selectedAddress)) {
                ...
                if (!empty($context->customer->id)) {
                    ...
                    $AddressObject->firstname = pSQL($ap_name);
                    $AddressObject->lastname = pSQL($ap_name);
                    $AddressObject->address1 = pSQL($ap_address1);
                    $AddressObject->company = pSQL($ap_address2);
                    $AddressObject->postcode = pSQL($ap_postcode);
                    $AddressObject->city = pSQL($ap_city);
                    $AddressObject->id_country =(int)$addressCustomer->id_country; 
                    $AddressObject->alias = pSQL(substr($ap_name, 0, 30));
                    $AddressObject->add();
                            Db::getInstance()->update(
                                'orders',
                                array('id_address_invoice' => $AddressObject->id),
    ...
    {
        // Accesspoint Address as SHIP-TO Address
        $context = Context::getContext();
        ...
        if (($param['order']->id_carrier)) {
            ...
            $selectedAddress = ($cook->__isset('selectedAcessPointAddress')) ?
                $cook->selectedAcessPointAddress : '';   
            $packageDimension = ($cook->__isset('package')) ?
                $cook->package : '';
            if (empty($shippingService)) {
                $shippingService = Configuration::get('UPS_SELECTED_SHIPPING_SERVICE');
            }
            ...
            if ($this->checkServiceType($shippingService) == 'AP' && !empty($selectedAddress)) {
                ...
                if (!empty($context->customer->id)) {
                    ...
                    // $context->cart->updateDeliveryAddressId($context->cart->id_address_invoice, $AddressObject->id);
                    // $context->cart->id_address_invoice = $AddressObject->id;    
                        // Db::getInstance()->update(
                        //     'orders',
                        //     array('id_address_invoice' => $AddressObject->id),
                        //     '`id_order` = ' . (int) $param['order']->id       
                    ...
                    // Db::getInstance()->execute("UPDATE "."`"._DB_PREFIX_."orders` SET `id_address_invoice`=".$AddressObject->id." WHERE `id_order` ='".(int) $param['order']->id."'");
                } 
                       $context->cart->updateAddressId($context->cart->id_address_invoice, $AddressObject->id);
            } else {
                $ap_id       = '';
                $ap_name     = '';
                $ap_address1 = '';
        ...
        // $cook->__unset('selectedAcessPointAddress'); // old code
        Configuration::updateValue('UPS_SELECTED_ACESSPOINT_ADDRESS', '');
        // $cook->__unset('selectedShippingService'); // old code
    ...
    {
        // $process_order= $this->CreateUPSLabel($params);
    ...
    public function CreateUPSLabel($params)
    {
        ...
        if (empty($order_id)) {
            ...
            $ui_output = "";
            require_once dirname(__FILE__) . '/controllers/admin/AdminUpsOpenOrdersController.php';
            require_once dirname(__FILE__) . '/common/CommonFunction.php';
        
            $openordercontroller_obj = new AdminUpsOpenOrdersController();
            $commonfunction_obj = new CommonFunction();
            ...
            $filtered_services = array();
                ...
                if ((bool)Tools::isSubmit('ups_label') == true) {
                $ship_info = $openordercontroller_obj->Processorder($order_id);
                
                if (isset($ship_info['errors']) && empty($ship_info['errors']))
                {
                    $selcted_service = !empty(Tools::getValue('ups_services_radio')) ? $this->getLabelservice(Tools::getValue('ups_services_radio'), $filtered_services) : [];
                    
                    if(empty($selcted_service)) {
                   $selcted_service = isset($filtered_services[(int)Tools::getValue('ups_services')]) ? $filtered_services[(int)Tools::getValue('ups_services')] : '';
                   }
                    $ship_info['shippingServiceInfo'] = $selcted_service;
                    $ship_res = $openordercontroller_obj->createShipment($ship_info);
                    
                    if (isset($ship_res['Description']) && $ship_res['Description'] == "Success") {
                            $ui_output = $this->displayConfirmation('Shipment Created Successfully');
                            $commonfunction_obj->sqlUpdateOrderStatus(4, [$order_id]);
                        } elseif (isset($ship_res['Description']) && $ship_res['Description'] != "Success")
                        {
                            $ui_output = $this->displayError($ship_res['Description']);
                        } else {
                            $ui_output = $this->displayError("Shipment creation failed.");
                        }
                    } else {
                        $ui_output = $this->displayError($ship_info['errors']);
                    }
                }

            $shipment_info = $this->getShipInfoByOrder($order_id);
            $label_available = (!empty($shipment_info)) ? true : false;
            
        if ((bool)Tools::isSubmit('ups_print_label') == true) {
            $args = array();
            $args['sdk'] = $this->sdk;
            $args['trackingNumber'] = $shipment_info[0]['tracking_number'];
            $args['labelFormat'] = Tools::getValue('ups_label_format');
            $ui_output = $this->downloadlabel($args);
        }

        $view_dir = _MODULE_DIR_ . $this->name . '/views';
       
        $this->context->smarty->assign('label_available', $label_available);
        $this->context->smarty->assign('all_ups_services', $filtered_services);
        $this->context->smarty->assign('order_id', $order_id);
        $this->context->smarty->assign('view_dir', $view_dir);
        
        $ui_output .= $this->context->smarty->fetch($this->local_path . 'views/templates/admin/ups_shipments/download_upslabel.tpl');
        return $ui_output;
    }

    public function getLabelservice($service_data, $service_list) {
        $service_data_arr = !empty($service_data) ? explode("-", $service_data) : [];
        $service_code = isset($service_data_arr[0]) ? $service_data_arr[0] : "";
        $service_name = isset($service_data_arr[1]) ? $service_data_arr[1] : "";//Service key
        $selected_service = [];
        
        if (!empty($service_code) && !empty($service_name) && !empty($service_list)) {
            foreach ($service_list as $service) {
                if (($service['key'] == $service_name) && ($service['Ratecode'] == $service_code)) {
    ...
    public function getShipInfoByOrder($idOrder)
    {
        $sql = new DbQuery();
        $sql->select('id');
    ...
    public function downloadlabel($args)
    {
        $objLabel = new Ups\Api\LabelRecovery();
        $res = $objLabel($args);
        $fileExt = '.' . Tools::strtolower($args['labelFormat']);
        $decoded = '';
        if (isset($res['LabelResults']) && !empty($res['LabelResults'])) {
            ...
            $tempName = @tempnam('tmp/', Constants::NAME_LABEL_SHIPMENT . '_');
            file_put_contents($tempName, $decoded);

            $filePaths = [];
            $filePaths[] = array(
            'srcPath' => $tempName,
            'desPath' => Constants::NAME_LABEL_SHIPMENT . '_' . $args['trackingNumber'] . $fileExt,
             );

            // $zipFilePath = @tempnam('tmp/', Constants::NAME_LABEL_SHIPMENT . '_');
            ...
            // }
            $fileDownload = Constants::NAME_LABEL_SHIPMENT . $fileExt;
            ...
            return $this->displayError("No label found.");
        }
    }
    ...
    public function doHandShake()
    {
        ...
        $data = array(
            ...
            'data' => array(
                ...
                // "VerboseResponseSecurityKey" => "I'm a developer",
    ...
    private function createContextSmarty()
    {
        ...
        $idAddressDelivery = $context->cart->id_address_delivery;
        // Set package dimension info before call rate api
        $this->setPackageDimension();
    ...
    private function makeListShippingServices(
            ...
            if (!empty($listServiceConfig)) {
                foreach ($listServiceConfig as $item) {
                    ...
                    if (!empty($rateData)) {
                        ...
                        // if (isset($rateData->ItemizedCharges)) {
                        //     $ItemizedCharges = $rateData->ItemizedCharges;
                           
                        //     if (is_array($ItemizedCharges) && count($ItemizedCharges) > 1) {
                        //         foreach ($ItemizedCharges as $ItemizedCharge) {
                        //             $apiMonetaryValue += $ItemizedCharge->MonetaryValue;
                        //         }
                        //     } else {
                        //         $apiMonetaryValue += $ItemizedCharges->MonetaryValue;
                        //     }
                        // }
        ...
        } elseif ($serviceType == "ADD") {
            $backupRateCodeList = [
                '110548','111030','111031','111035','111036','111050', '111055',
                '111056','111057','111546','111547','111548', '112117','112118',
                '112119','112120','9110054','9110055','9110056','9110057','9110023'
            ];
            if (isset($apiResponseService->Code) && in_array($apiResponseService->Code, $backupRateCodeList)) {
                $packageSettingType = Configuration::get('UPS_PKG_SETTING_TYPE');
                if ($packageSettingType == 2) {
                    $productDimension = unserialize(Configuration::get('UPS_PKG_BACKUP_RATE'));
                    if ($productDimension) {
                        foreach ($listServiceConfig as $item) {
                            foreach ($productDimension as $backupRate) {
                                if ($backupRate['service_key'] == $item['id_service']) {
                                    $service = [];
                                    $shippingFeeValue = (float)round($backupRate['rate'], 2);
                                    $service['id_service'] = $item['id_service'];
                                    $service['name'] = $item['name'];
                                    $service['totalTransitDays'] = '';
                                    $service['detailTime'] = '';
                                    $service['shippingFeeValue'] = $shippingFeeValue;
                                    $service['detailFee'] = Tools::displayPrice($shippingFeeValue);
                                    $service['priceDisplay'] = Tools::displayPrice($shippingFeeValue);
                                    $showShippingService[] = $service;
                                }
                            }
                        }
                    }
    ...
    private function transDaysOfTotalDays($stringDays)
    {
        ...
        $dayTime = date('d', strtotime($stringDays));
        $monthTime = $this->sdk->t('ups', date('F', strtotime($stringDays)));
        $yearTime = date('Y', strtotime($stringDays));
        $transDaysOfTotalDays = sprintf("%s %s %s", $dayTime, $monthTime, $yearTime);
    ...
    private function callRateAPI(
        ...
        $addressCustomer = new Address($idAddressDelivery);
        $packageInfo = unserialize($this->context->cookie->__get('package'));
        ...
        if ($requestOption == Constants::RATE_API_REQ_OPT_TIME || $requestOption == Constants::RATE_API_REQ_OPT_STIT) {
            $rateInfo['PackageWeightCode']        = isset($packageInfo[0]['unit_weight']) ? $packageInfo[0]['unit_weight'] : '';
            $rateInfo['PackageWeightDescription'] = isset($packageInfo[0]['unit_weight']) ? $packageInfo[0]['unit_weight'] : '';
            $rateInfo['PackageWeightWeight']      = isset($packageInfo[0]['weight']) ? $packageInfo[0]['weight'] : '';
    ...
    private function removeConfigPkgDimension()
    {
        ...
        Configuration::deleteByName('UPS_PKG_SETTING_TYPE');
        Configuration::deleteByName('UPS_PKG_DEFAULT');
        Configuration::deleteByName('UPS_PKG_INCLUDE_DIMENSION');
        Configuration::deleteByName('UPS_PKG_PRODUCT_DIMENSION');
        Configuration::deleteByName('UPS_PKG_BACKUP_RATE');
    }
     ...
     *
     */
    private function setPackageDimension()
    {
        ...
        if ($packageSettingType == false || $packageSettingType == $packageDimension::TYPE_PACKAGE_DEFAULT) {
            ...
            $numberOfCartItem = $this->context->cart->nbProducts();
            ...
            if ($selected_pac_algo == 1 || $selected_pac_algo == 2) {
                $pack_weg_unit = Configuration::get('UPS_PKG_WEIG_UNIT');
                if($selected_pac_algo == 2) { //Weight Based  Packing
                    $pack_max_weight = Configuration::get('UPS_PKG_MAX_WEG');
                    $listPackages = $packageDimension->weight_pack($cartProduct, $pack_weg_unit, $pack_max_weight);
                } else { //Pack Item Indidividually
                    $pack_dim_unit = Configuration::get('UPS_PKG_DIM_UNIT');
                    $listPackages = $packageDimension->per_item($cartProduct, $pack_weg_unit, $pack_dim_unit);
                }
            } elseif ($selected_pac_algo == 3) { //Box Based Packing
                $listProductDimension = unserialize(Configuration::get('UPS_PKG_PRODUCT_DIMENSION'));
                $listPackages = $packageDimension->box_pack($cartProduct, $listProductDimension);
        ...
        $this->context->cookie->__set('package', serialize($listPackages));
